...
void PlayScene::update()
{
	...
	CollisionManager::AABBCheck(m_pDog, m_pObstacle);
	CollisionManager::AABBCheck(m_pDog, m_pTarget);
}
...
void PlayScene::start()
{
	...
	m_pDog = new Dog();
	m_pDog->getTransform()->position = glm::vec2(100.0f, 300.0f);
	m_pDog->setEnabled(false);
	m_pDog->setDestination(m_pTarget->getTransform()->position);
	addChild(m_pDog);
}
...
void PlayScene::GUI_Function() const
{
	...
	{
		m_pDog->setMaxSpeed(speed);
	}

	static float acceleration_rate = 2.0f;
	if(ImGui::SliderFloat("Acceleration Rate", &acceleration_rate, 0.0f, 50.0f))
	{
		m_pDog->setAccelerationRate(acceleration_rate);
	}

	static float angleInRadians = m_pDog->getRotation();
	if(ImGui::SliderAngle("Orientation Angle", &angleInRadians))
	{
		m_pDog->setRotation(angleInRadians * Util::Rad2Deg);
	}

	static float turn_rate = 5.0f;
	if(ImGui::SliderFloat("Turn Rate", &turn_rate, 0.0f, 20.0f))
	{
		m_pDog->setTurnRate(turn_rate);
	}
	
	if(ImGui::Button("Start"))
	{
		m_pDog->setEnabled(true);
	}
	...
	{
		m_pDog->getTransform()->position = glm::vec2(100.0f, 100.0f);
		m_pDog->setEnabled(false);
		m_pDog->getRigidBody()->velocity = glm::vec2(0.0f, 0.0f);
		m_pDog->setRotation(0.0f); // set Angle to 0 degrees
		turn_rate = 5.0f;
		acceleration_rate = 2.0f;
		speed = 10.0f;
		angleInRadians = m_pDog->getRotation();
	}
